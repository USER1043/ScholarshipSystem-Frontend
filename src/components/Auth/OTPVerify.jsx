...
const OTPVerify = () => {
  const { verifyOtp, resendOtp, getOtpStatus } = useContext(AuthContext);
  ...
  // Fetch actual remaining time from server on mount
  useEffect(() => {
    const fetchStatus = async () => {
      if (userId) {
        const data = await getOtpStatus(userId);
        if (data && typeof data.remainingTime === "number") {
          setTimer(data.remainingTime);
          if (data.remainingTime === 0) setCanResend(true);
        }
      }
    };
    fetchStatus();
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      // If backend returns remainingTime, sync timer
      if (err.response?.data?.remainingTime) {
        setTimer(err.response.data.remainingTime);
        setCanResend(false);
      }
    }
  };
